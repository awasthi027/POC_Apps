...
class CTKTokenListViewModel: ObservableObject {
    ...
    func secIdentity(certificate: CertificateInfo) -> SecIdentity? {
        for identity in self.allIdentities {
            guard let certData = identity.secCertificateAndDataFromSecIdentity?.1 else { continue }
            let targetCert = CertificateInfo(certificateData: certData)
            if certificate.serialNumber == targetCert.serialNumber {
                return identity
            }
        }
        return nil
...
extension SecIdentity {
    ...
    var secCertificateAndDataFromSecIdentity: (SecCertificate, Data)? {
        var secCert: SecCertificate? = nil
        let status = SecIdentityCopyCertificate(self, &secCert)
        if status == noErr {
            print("[Consumer] Certificate identity fetch status : \(status)")
            guard let cert = secCert else {
                print("[Consumer] Certificate not found")
            ...
            let x509Cert = SecCertificateCopyData(cert) as Data
            guard let certificate = SecCertificateCreateWithData(kCFAllocatorDefault, x509Cert as CFData) else {
                return nil
            }
            return(certificate, x509Cert)
        }
        return nil
    }
}
