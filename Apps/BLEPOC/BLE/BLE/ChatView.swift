    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .principal) {
                NavTitleWithStatus(title: "Chat", isOnline:  self.service.isBLEAdvertising)
            }
            ToolbarItem(placement: .navigationBarTrailing) {
                Button(action: { self.service.isBLEAdvertising.toggle() }) {
                    Image(systemName: self.service.isBLEAdvertising ? "wifi" : "wifi.slash")
                        .foregroundColor(self.service.isBLEAdvertising ? .green : .red)
                }
